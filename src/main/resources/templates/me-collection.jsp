...
<body>
	...
	<div class="content">
		...
		<c:if test="${goodsCollectionList == null}">
			什么都没有呢X﹏X
		</c:if>
		<c:forEach var="collectionItem" items="${goodsCollectionList}">
			<div class="content-item">
				<div class="content-item-body">
					<div class="content-item-body-item1 fl">
						<div class="content-item-body-item1-img fl" style="overflow: hidden;">
							<a href="/TradingPlatform/goods/single/${collectionItem.goods.goodsId}"><img width="80" th:src="@{/upload/goodsImg/${collectionItem.goods.goodsImg}/1.jpg"></a>
						</div>
					...
					<div class="content-item-body-item2 fr">
						<a href="/TradingPlatform/goods/single/${collectionItem.goods.goodsId}"><img width="26" th:src="@{/images/edit.png"></a>
						<a href="/TradingPlatform/goods/me-collection/delete/${collectionItem.goods.goodsId}/${pageNum}"><img width="26" th:src="@{/images/delete.png"></a>
					</div>
					<div class="content-item-body-item2 fr">
						<c:choose>
							<c:when test="${collectionItem.goods.state == 1}">未售出</c:when>
		...
		<div class="page">
			<c:if test="${totalPages>=1}">
				...
				<div class="page-item fl bg-color-fff">
					<a href="/TradingPlatform/goods/me-collection/${pageNum-1}"><img alt="上一页" th:src="@{/images/prevPage.png"></a>
				</div>
				<%-- 首页 --%>
				<div class="page-item fl <c:if test="${pageNum==1}">bg-color-ffd84d</c:if>" style="background-color: #f2f2f2">
					<a href="/TradingPlatform/goods/me-collection/1">1</a>
				</div>
				<%-- 页数过多时的省略号 --%>
					...
					<c:when test="${totalPages<=9}">
						<c:forEach var="pagesNum" begin="2" end="${totalPages-1}">
							<div class="page-item fl <c:if test="${pagesNum==pageNum}">bg-color-ffd84d</c:if>" style="background-color: #f2f2f2">
								<a href="/TradingPlatform/goods/me-collection/${pagesNum}">${pagesNum}</a>
							</div>
						</c:forEach>
					</c:when>
					<c:when test="${totalPages > 9}">
				...
				<c:if test="${totalPages > 1}">
					<div class="page-item fl <c:if test="${pageNum == totalPages}">bg-color-ffd84d</c:if>" style="background-color: #f2f2f2">
						<a href="/TradingPlatform/goods/me-collection/${totalPages}">${totalPages}</a>
					</div>
				</c:if>
				<%-- 下一页 --%>
				<div class="page-item fl bg-color-fff">
					<a href="/TradingPlatform/goods/me-collection/${pageNum+1}"><img alt="下一页" th:src="@{/images/nextPage.png"/></a>
				</div>
				<%-- 页数较少时填充 --%>
